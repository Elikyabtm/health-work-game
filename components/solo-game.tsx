...
export default function SoloGame({ config, onGameEnd, onBack }: SoloGameProps) {
  ...
  const startNewRound = () => {
    const { keyword, words, definition } = getRandomKeyword(config.theme)
  ...
  const endRound = () => {
    ...
    // Utiliser la nouvelle fonction de correspondance tolérante
    const matchedWords = findMatchingWords(playerWords, expectedWords)
    ...
    const roundResult = {
      ...
      playerAnswers: { solo: playerWords },
      commonWords: matchedWords,
      scores: { solo: roundScore },
      expectedWords,
    }
    ...
    // Afficher le bouton suivant au lieu d'un timer automatique
  ...
  const goToNextRound = () => {
    if (currentRound < config.numberOfRounds) {
      ...
      const finalSession: GameSession = {
        id: "solo-" + Date.now(),
        config,
        players: [{ id: "solo", name: "Joueur", score: totalScore }],
        currentRound,
        currentKeyword,
        expectedWords,
        roundResults,
  ...
  // Utiliser la nouvelle fonction de correspondance
  const matchedWords = findMatchingWords(playerWords, expectedWords)
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold">
              Manche {currentRound}/{config.numberOfRounds}
            </h1>
              ...
              {roundActive ? (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-gray-600">
                    💡 Écris tous les mots qui te viennent à l'esprit en lien avec "{currentKeyword}" pour trouver sa
                    définition !
                  </p>
                ...
                <div className="text-center py-8">
                  ...
                  <div className="text-sm text-gray-600 mb-4">
                    ...
                    {matchedWords.length > 1 ? "s" : ""} sur {expectedWords.length}
                  </div>
                  {showNextButton && (
              ...
              <div className="space-y-4 max-h-64 overflow-y-auto">
                ...
                {!roundActive && (
                  <div className="mt-4 p-3 bg-green-50 rounded-lg">
                    <div className="text-sm font-medium text-green-800">
                      ✅ Mots corrects : {matchedWords.length}/{expectedWords.length}
                    </div>
                ...
                {showExpected && !roundActive && (
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                    <div className="text-sm font-medium text-blue-800 mb-2">💡 Mots attendus :</div>
